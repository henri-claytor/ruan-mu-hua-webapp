...
- **THEN** weightedDaily60 為空陣列；calcMultiScaleEV 內部判斷 short = null；醫尺度結果 short 欄位為 null，divergence 退化為 stable
...
- **THEN** 渲染 `<MultiScaleEVBlock result={evMulti} monthlyCount={portMonthly.length} dailyCount={minDailyCount} titleOverride="組合期望報酬與賠率優勢" />`
