...
- **THEN** 回傳 6 元素陣列，每元素包含 `{ bucket: string, wins: number, losses: number, avgWinReturn: number, avgLossReturn: number }`
...
#### Scenario: 容器內排版
- **WHEN** 容器展開
- **THEN** 由上至下依序顯示：累積損益曲線（全寬）→ 個股貢獻長條 + 持有天數分佈（桌機並排，手機 stack）

#### Scenario: 交易筆數不足時容器顯示提示
- **WHEN** trades.length < 5
- **THEN** 容器仍渲染，但圖表下方顯示「交易筆數較少（< 5 筆），圖表參考度有限」說明
