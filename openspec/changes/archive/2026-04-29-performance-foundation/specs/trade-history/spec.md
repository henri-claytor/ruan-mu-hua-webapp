...
- **THEN** 將 newTrades 加入既有 `trades` 陣列尾部（不去重）
...
#### Scenario: 新增空白列
- **WHEN** 使用者點擊「+ 新增交易」
- **THEN** 表格底部新增一空白列，可填入 13 欄（部分欄位有預設值如今日 / 0）
...
#### Scenario: CSV 格式定義
- **WHEN** 使用者上傳 CSV
- **THEN** 系統期望檔案首列為 header，包含 13 欄（與 Trade 介面對應）：`stock_id, stock_name, buy_date, sell_date, buy_price, sell_price, shares, buy_amount, sell_amount, pnl, return_rate, note`
...
#### Scenario: 報酬率自動偵測
- **WHEN** CSV 的 return_rate 值大於 1（如 24.03）
- **THEN** 系統視為百分比格式，自動除以 100；小於等於 1（如 0.2403）視為小數格式
