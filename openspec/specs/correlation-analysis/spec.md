...
提供持有天數與報酬率的 Pearson 相關係數計算與散布圖視覺化，作為績效分析的延伸觀察，幫助使用者判斷自身策略是否依賴特定持有期間。
...
- **THEN** 元件顯示「樣本不足無法計算相關性（需要至少 5 筆）」訊息，不渲染圖表
...
#### Scenario: 點配色紅綠跌
- **WHEN** 渲染散布圖點
- **THEN** `returnRate > 0` 紅色（`CHART_COLORS.positive`）、`returnRate < 0` 綠色（`CHART_COLORS.negative`）、`returnRate === 0` 中性灰

#### Scenario: r 值顯示
- **WHEN** 散布圖渲染
- **THEN** 圖表上方顯示「相關係數 r = X.XX | <解讀文字>」

#### Scenario: 軸標籤
- **WHEN** 圖表渲染
- **THEN** x 軸標籤「持有天數」、y 軸標籤「報酬率 (%)」、區塊標題「持有天數 vs 報酬率」

#### Scenario: Tooltip 樣式
- **WHEN** 滑鼠 hover 某點
- **THEN** Tooltip 顯示「{stockName} {stockId} | 持有 X 天 · 報酬 ±Y.YY%」，樣式沿用 `utils/chartStyle.ts` 的 `TOOLTIP_STYLE`
